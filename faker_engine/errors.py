...
class FakerEngineError(Exception):
    """Base exception for all faker_engine errors.
    ...
    Examples:
        >>> raise FakerEngineError("something went wrong")
        Traceback (most recent call last):
        ...
        FakerEngineError: something went wrong

        >>> raise FakerEngineError("missing field", path="fields.event_date")
        Traceback (most recent call last):
        ...
        FakerEngineError: [fields.event_date] missing field
    """

    def __init__(self: object, message: Optional[str]=None, path: tuple[str, ...] | None=None) -> None:
        """Init _.
...
class ContextError(FakerEngineError):
...
class ConfigError(FakerEngineError):
...
class SpecError(FakerEngineError):
...
class APIError(FakerEngineError):
