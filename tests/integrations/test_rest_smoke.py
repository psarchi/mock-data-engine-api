...
def test_chaos_ops(chaos_op, rest_baseline, rest_chaos):
    """Test each chaos op applies correctly via REST endpoint.
    ...
    test_func_name = f"test_{chaos_op}_rest"
    ...
    chaos_result = rest_chaos(chaos_op)

    baseline_items = rest_baseline["items"]
    ...
    if "baseline_elapsed" in sig.parameters:
        kwargs["baseline_elapsed"] = rest_baseline["elapsed"]
    if "chaos_elapsed" in sig.parameters:
        kwargs["chaos_elapsed"] = chaos_result["elapsed"]
    if "status_code" in sig.parameters:
        kwargs["status_code"] = chaos_result.get("status_code")
...
def test_schema_validation(rest_baseline):
