...
class HeaderAnomalyOp(BaseChaosOp):
    ...
    def __init__(self, *, enabled: bool, p: float = 0.0, weight: float = 1.0,
                 patterns: str = "huge-value", huge_value_bytes: int = 2048,
                 dup_keys_count: int = 3, **kw) -> None:
    ...
    def apply(self, *, request, response, body: Any,
        ...
        try:
            if "huge" in str(self.patterns):
                ...
                val = "".join(
                    chr(int(rng.random() * 26) + 65) for _ in range(size))
            ...
            if self.dup_keys_count > 1:
                for i in range(self.dup_keys_count):
                    response.headers.add_vary("X-Dup-Key") if hasattr(
                        response.headers,
                        "add_vary") else response.headers.__setitem__(
                        f"X-Dup-Key-{i}", "1")
